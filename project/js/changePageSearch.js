...
function getKeyContent() {
    ...
    let keycontent = linkString.split("=")[1];
    keycon = decodeURI(keycontent);
}
getKeyContent();
// 设置用户信息和头像
...
// 获取最大长度
(function() {
    let searchContentLengthJSON = {
        "uid": getCookie("id"),
        "key": keycon,
    }
    let searchContentLengthJSONString = JSON.stringify(searchContentLengthJSON);
    let lengthData = getJSON('POST', 'http://175.178.51.126:8091/smallA/selectDiary', searchContentLengthJSONString).then(res => {
        let tips = JSON.parse(res);
        maxLength = tips.data.length;
        console.log("1234:" + tips);
